...
def _comma_format(var):
    """StringVar 값을 '숫자만 + 천단위 콤마' 정수 표기로 정규화.
    값이 바뀌면 var.set() 후 True 반환(트레일 재진입으로 recalc 처리)."""
...
class EventCalc(ttk.Frame):
    ...
    def __init__(self, master, event, store):
        ...
        self.point_vars = []
        self.need_labels = []
        self._vcmd = (self.register(_is_number), "%P")  # 숫자만 입력 허용
        ...
        self.current_var.trace_add("write", lambda *a, v=self.current_var: _comma_format(v) or self.recalc())
        _comma_format(self.current_var)
        ...
        self.target_var.trace_add("write", lambda *a, v=self.target_var: _comma_format(v) or self.recalc())
        _comma_format(self.target_var)
...
class CustomEventCalc(ttk.Frame):
    ...
    def recalc(self):
        ...
        for it, lbl in zip(self.items, self.need_labels):
            pts = self._to_num(it.get("points", "0"))
            if target <= 0 or gap <= 0:
                lbl.config(text="0개" if gap <= 0 and target > 0 else "-")
            elif pts <= 0:
                lbl.config(text="—")
